...
class SemanticBranch(nn.Module):
    ...
    def forward(self, x: torch.Tensor, inference: bool = True) -> Union[torch.Tensor, tuple[torch.Tensor, list[torch.Tensor]]]:
        x1 = self.s1_s2(x)
        x2 = self.s3(x1)
        x3 = self.s4(x2)
        x4 = self.s5_4(x3)
...
class BiSeNetV2(
    ...
    def forward(self, x: torch.Tensor, inference: bool = True):
        ...
        if inference:
            semantic_output = self.semantic_branch(x, inference) # H/32 x W/32 x C
